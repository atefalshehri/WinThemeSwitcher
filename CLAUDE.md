...
- `release.yml` — on tag push `v*` (or manual dispatch): builds on GitHub runners and attaches a zip (exe + README + LICENSE + publisher `.cer`) plus the bare exe and `WinThemeSwitcher-publisher.cer` to a **prerelease**. The `.cer` is committed at the repo root (public cert only — byte-identical to the store cert's export). **CI binaries are unsigned** — the signing key exists only on this machine. Verified 2026-07-04: v0.3.0's GitHub binaries (zip exe and bare exe) are both unsigned CI builds; only the `.cer` was uploaded manually, and the signed v0.3.0 binary exists only in the local deploy at `C:\Tools\`. The README's "signed builds since v0.3.0" note therefore overstates the GitHub assets. To make a release genuinely signed: build locally, sign (section above), then replace the assets with `gh release upload <tag> <files> --clobber`.
